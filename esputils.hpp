...
namespace var
{
	...
	namespace esp {
		...
		bool animals = false;
		bool hide = false; // was true: gates ALL ESP behind a byte read at player_input+0x94 which is unreliable across builds
...
void ESP_stringset ( T& vec )
{
	...
	for ( const auto& entry : vec )
	{
		...
		Vector3 position = rust::get_entity_world_position ( mist::esp.get ( ), entity );
		if ( position.empty ( ) )
			continue;
...
void ESP_player ( T& vec )
{
	...
	for ( const auto& entry : vec )
	{
		...
		// NeoRed v6: legacy `get_object_pos_component+0x90` is dead on Unity 6.
		// Use the bone-walker derived pelvis position instead (works because
		// we fixed the access_struct_off=0x28 issue in unity.cpp).
