...
void object::setup ( )
{
	...
	if ( addr::gameassembly != 0 )
		logging::print ( TYPE_DEBUG, "UnityPlayer.dll: 0x%p", (void*)addr::unityplayer );
	else
		logging::print ( TYPE_ERROR, "Couldn't find UnityPlayer.dll!" );
	if ( addr::unityplayer != 0 )
		logging::print ( TYPE_DEBUG, "GameAssembly.dll: 0x%p", (void*)addr::gameassembly );
	else
		logging::print ( TYPE_ERROR, "Couldn't find GameAssembly.dll!" );

	logging::print ( TYPE_DEBUG, "Object list: 0x%p (%zu)", (void*)addr::object_list, addr::object_size );
...
void object::refresh_game ( )
{
	...
	while ( true )
	{
		...
		uintptr_t cam = rust::resolve_main_camera ( mist::obj.get ( ) );

		// Refresh local player from the static-slot fast path every tick. This
		// is cheap (3 reads) and keeps `m_pLocalplayer` valid even if the entity
		// walk is empty.
		{
			uintptr_t static_local = rust::get_local_player_static ( mist::obj.get ( ) );
			if ( static_local )
			{
				game::m_pLocalplayer = (BasePlayer*)static_local;
				game::m_rLocalInfo.m_pPlayer = static_local;
			}
		}
		...
		if ( !diagnostics_done )
		{
			if ( cam )
			{
				...
				Vector3   campos = rust::get_camera_position ( mist::obj.get ( ) );
				Matrix4x4 vm     = rust::get_view_matrix     ( mist::obj.get ( ) );
				logging::print ( TYPE_DEBUG, "camera_instance = 0x%llX  cam_pos=(%.1f,%.1f,%.1f)",
					(unsigned long long)cam, campos.x, campos.y, campos.z );
				logging::print ( TYPE_DEBUG, "view_matrix row0 = (%.3f, %.3f, %.3f, %.3f)",
					vm[0][0], vm[0][1], vm[0][2], vm[0][3] );
				logging::print ( TYPE_DEBUG, "view_matrix row2 = (%.3f, %.3f, %.3f, %.3f)",
					vm[2][0], vm[2][1], vm[2][2], vm[2][3] );
				logging::print ( TYPE_DEBUG, "view_matrix row3 = (%.3f, %.3f, %.3f, %.3f)  (expect m[3][2] ~= -1)",
					vm[3][0], vm[3][1], vm[3][2], vm[3][3] );
			}
			else
			{
				logging::print ( TYPE_WARNING, "[refresh_game] resolve_main_camera returned 0 - retrying next tick" );
			}
		}
...
void raid_esp ( )
{
	if ( !addr::gameassembly )
		return;
	const uintptr_t m_pEffectClass = mist::obj->read<uintptr_t> (
		addr::gameassembly + offsets::effect_network_c );
	if ( !m_pEffectClass )
		return;
...
void object::main ( )
{
	// Wait until EITHER the entity walk produced a non-empty list OR the
	// NeoRed v6 static-slot path resolves a local BasePlayer. The latter works
	// even when the entity walk is broken (e.g. wrong decryption/handle scheme)
	// and lets ESP / aim run on the local player only as a graceful fallback.
	{
		uintptr_t static_local = 0;
		int waited_ms = 0;
		while ( addr::object_size == 0 && static_local == 0 )
		{
			static_local = rust::get_local_player_static ( mist::obj.get ( ) );
			if ( static_local )
			{
				game::m_pLocalplayer = (BasePlayer*)static_local;
				game::m_rLocalInfo.m_pPlayer = static_local;
				logging::print ( TYPE_DEBUG,
					"[fast-path] local player resolved via static slot: 0x%llX",
			...
			waited_ms += 50;
			if ( waited_ms == 5000 )
				logging::print ( TYPE_WARNING,
					"[main] still waiting for entities or local player - are you in a server?" );
		}
	}

	// TOD_Sky resolution from NeoRed v6 dump:
	//   klass = read(GA + 0xE2EF500)
	//   sf    = read(klass + 0xB8)
	//   tod   = read(sf + 0x80)
	{
		uintptr_t tod_klass = mist::obj->read<uintptr_t> ( addr::gameassembly + 0xE2EF500ULL );
		uintptr_t tod_sf    = tod_klass ? mist::obj->read<uintptr_t> ( tod_klass + 0xB8 ) : 0;
		addr::tod_sky       = tod_sf    ? mist::obj->read<uintptr_t> ( tod_sf + 0x80 )    : 0;
	}
	...
	while ( true )
	{
		...
		// === Local player held-weapon resolution (unconditional) ============
		// The legacy `i == 0` gate in the entity walk no longer fires on
		// Unity 6 because the local player is not at index 0. Resolve it here
		// using the cached pointer (filled by `refresh_game` via the static
		// slot) so weapon name / prediction / speed / gravity always get
		// populated whenever a local player exists.
		if ( game::m_pLocalplayer )
		{
			...
			if ( game::m_rLocalInfo.m_pWeapon != weapon )
			{
				...
				game::m_rLocalInfo.m_szWeaponName  = weapon ? weapon->get_name        ( mist::obj.get ( ) ) : std::string ( );
				game::m_rLocalInfo.m_szBulletType  = weapon ? weapon->get_bullet_name ( mist::obj.get ( ) ) : std::string ( );
				...
				logging::print ( TYPE_DEBUG,
					"[held] weapon=%p name='%s' bullet='%s' predict=%d speed=%.1f grav=%.2f",
					(void*)weapon,
					game::m_rLocalInfo.m_szWeaponName.c_str ( ),
					game::m_rLocalInfo.m_szBulletType.c_str ( ),
		...
		// One-shot dump of the first 16 entries so we can see what's actually in the
		// entity list, what their tag is, and what class name they resolve to.
		static bool s_first_dump_done = false;
		if ( !s_first_dump_done && addr::object_size > 0 )
		{
			s_first_dump_done = true;
			logging::print ( TYPE_DEBUG, "=== entity list snapshot (first 16 of %zu) ===", addr::object_size );
			const uint64_t dump_n = ( addr::object_size < 16ULL ) ? addr::object_size : 16ULL;
			int tag6_total = 0;
			for ( uint64_t i = 0; i < dump_n; i++ )
			{
				uintptr_t element = mist::obj->read<uintptr_t> ( addr::object_list + 0x20 + ( i * 0x08 ) );
				if ( !element )
				{
					logging::print ( TYPE_DEBUG, "  [%llu] element=NULL", (unsigned long long)i );
					continue;
				}
				std::string cname = rust::get_class_name ( mist::obj.get ( ), element );
				uintptr_t bmo    = mist::obj->read<uintptr_t> ( element + 0x10 );
				uintptr_t obj    = bmo ? mist::obj->read<uintptr_t> ( bmo + 0x30 ) : 0;
				USHORT    tag    = obj ? mist::obj->read<USHORT> ( obj + 0x54 ) : 0xFFFF;
				logging::print ( TYPE_DEBUG, "  [%llu] elem=0x%llX bmo=0x%llX obj=0x%llX tag=%u cls='%s'",
					(unsigned long long)i, (unsigned long long)element,
					(unsigned long long)bmo, (unsigned long long)obj, (unsigned)tag,
					cname.c_str ( ) );
				if ( tag == 6 ) tag6_total++;
			}
			// also count tag==6 entries across the whole list so we know
			// roughly how many players exist.
			int total_players = 0;
			for ( uint64_t i = 0; i < addr::object_size; i++ )
			{
				uintptr_t element = mist::obj->read<uintptr_t> ( addr::object_list + 0x20 + ( i * 0x08 ) );
				if ( !element ) continue;
				uintptr_t bmo = mist::obj->read<uintptr_t> ( element + 0x10 );
				if ( !bmo ) continue;
				uintptr_t obj = mist::obj->read<uintptr_t> ( bmo + 0x30 );
				if ( !obj ) continue;
				if ( mist::obj->read<USHORT> ( obj + 0x54 ) == 6 )
					total_players++;
			}
			logging::print ( TYPE_DEBUG, "=== entities with tag==6 in full list: %d (first-16 had %d) ===",
				total_players, tag6_total );
		}
		...
		for ( uint64_t i = 0x0; i < addr::object_size; i++ )
		{
			...
			// The legacy bmo+0x30 / obj+0x30 chain is dead in modern Unity (returns
			// a static UnityPlayer.dll vtable for every entity). `element` IS the
			// managed BasePlayer instance, so use it directly as both the player
			// pointer and the m_pPlayer handle. We branch on the class-name hash
			// instead of the broken Unity tag.
			uintptr_t base_mono_object = mist::obj->read<uintptr_t> ( element + 0x10 );
			if ( !base_mono_object )
				continue;

			uintptr_t object      = base_mono_object;   // legacy alias - some entity branches use this
			uintptr_t objectClass = element;            // managed BasePlayer/etc

			const bool is_player_entity =
				element_hash == player_hash ||
				element_hash == sc1 || element_hash == sc2 || element_hash == sc3 ||
				element_hash == sc4 || element_hash == sc5 || element_hash == sc6;

			if ( is_player_entity )
			{
				BasePlayer* player = (BasePlayer*)element;
				if ( !player )
					continue;

				// On Unity 6 the local player is no longer guaranteed to sit at
				// entity index 0; `refresh_game()` resolves it via the static
				// slot. Match by pointer instead so the held-weapon block
				// actually executes for the local player.
				if ( player == game::m_pLocalplayer || (uintptr_t)objectClass == game::m_rLocalInfo.m_pPlayer )
				{
					...
					if ( game::m_rLocalInfo.m_pWeapon != weapon )
					{
						...
						// Position via the bone-transform path (the old pos-component
						// chain reads through the broken bmo+0x30 link).
						uintptr_t player_model = mist::obj->read<uintptr_t> ( (uintptr_t)player + offsets::model );
						uintptr_t bone_root    = player_model ? mist::obj->read<uintptr_t> ( player_model + offsets::bone_transforms ) : 0;
						if ( bone_root )
						{
							uintptr_t b0 = mist::obj->read<uintptr_t> ( bone_root + 0x20 ); // bone[0]
							if ( b0 )
							{
								uintptr_t b0_node = mist::obj->read<uintptr_t> ( b0 + 0x10 );
								if ( b0_node )
									game::m_rLocalInfo.m_vecLocal = unity::get_position_injected ( mist::obj.get ( ), b0_node );
							}
						}
				...
				else
				{
					...
					if ( game::cache.find ( uid ) == game::cache.end ( ) )
					{
						...
						cache.transform = mist::obj->read<uintptr_t> ( cache.player_model + offsets::bone_transforms );
					...
					game::cache[uid].transform = mist::obj->read<uintptr_t> ( game::cache[uid].player_model + offsets::bone_transforms );
			...
			else if ( var::esp::item && ( element_hash == item_hash || element_hash == corpse_hash ) ) // DroppedItem PlayerCorpse
			{
				...
				Vector3 position = rust::get_entity_world_position ( mist::obj.get ( ), objectClass );
				if ( position.empty ( ) )
					continue;
