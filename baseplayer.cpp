...
void BasePlayer::omni_sprint ( CSharedMemComms* drv )
{
	...
	if ( !m_vecVelocity.empty ( ) )
	{
		...
		model_state->set_sprinting ( drv, true );
		drv->write<Vector3> ( m_pMovement + offsets::target_movement, m_vecVelocity * 0.97f );
		model_state->set_sprinting ( drv, true );
	}
}
...
Vector3 BasePlayer::get_bone_position ( CSharedMemComms* drv, int bone, uintptr_t transform )
{
	...
	if ( transform == 0 )
	{
		...
		_transform = drv->read<uintptr_t> ( player_model + offsets::bone_transforms );
	}
	else
	...
	if ( !bones ) return {};
	// Use rust::get_bone_position style sanity check. The SSE walker in
	// unity::get_position_injected mallocs/dereferences based on memory-read
	// values; without bounds checks it crashes when transforms are torn or
	// not yet initialised (entity just spawned, etc.).
	return rust::get_bone_position_from_transform ( drv, _transform, reinterpret_cast<uintptr_t>( this ), bone );
}

Item* BasePlayer::get_held_item ( CSharedMemComms* drv )
{
	uint64_t active_id;
	active_id = drv->read<uint64_t> ( reinterpret_cast<uintptr_t>( this ) + offsets::active_item ); // clActiveItem (ulong)
	...
	for ( int i = 0; i < 6; i++ )
	{
		...
		CASSERT ( weapon );
		uint64_t uid = weapon->get_uid ( drv );
		if ( !uid )
			continue;
...
{
	uint64_t m_iActiveItem;
	m_iActiveItem = drv->read<uint64_t> ( reinterpret_cast<uintptr_t>( this ) + offsets::active_item );
	...
	for ( UINT i = 0; i < 6; ++i )
	{
		...
		m_piEntry.m_szName = m_pItem->get_name ( drv );
...
{
	PlayerEyes* player_eyes = this->get_player_eyes ( drv );
	if ( !player_eyes )
		return {};
	return drv->read<Vector2> ( reinterpret_cast<uintptr_t>( player_eyes ) + offsets::head_angles );
}
...
{
	uintptr_t m_pLocalTeam = drv->read<uintptr_t> ( reinterpret_cast<uintptr_t>( game::m_pLocalplayer ) + offsets::team ); ASSERT ( m_pLocalTeam ); // clientTeam
...
Vector3 BasePlayer::get_velocity ( CSharedMemComms* drv )
{
	...
	Vector3 velocity = drv->read<Vector3> ( playermodel + offsets::velocity ); // velocity
	return velocity;
}
...
Vector4 PlayerEyes::get_body_rotation ( CSharedMemComms* drv ) {
	return drv->read<Vector4> ( reinterpret_cast<uintptr_t>( this ) + offsets::bodyRotation );
}

void PlayerEyes::set_body_rotation ( CSharedMemComms* drv, Vector4 rotation )
{
	drv->write ( reinterpret_cast<uintptr_t>( this ) + offsets::bodyRotation, rotation );
}
